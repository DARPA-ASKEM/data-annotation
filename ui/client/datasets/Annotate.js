...
export default withStyles(({ spacing }) => ({
  ...
  useEffect(() => {
    ...
    const previewUrl = `/api/data_annotation/datasets/${datasetInfo.id}/preview/raw${rawFileName ? `?${fileArg}=${rawFileName}` : ''}`;
    ...
    const getAnnotations = async () => {
      ...
      if (annotations?.metadata?.geotime_classify) {
        ...
        return axios.get(`/api/data_annotation/datasets/${datasetInfo.id}/annotations`);
      }
    };
  ...
  function submitToBackend() {
    ...
    axios
      .patch(`/api/data_annotation/datasets/${datasetInfo.id}/annotations`, backendPayload)
      .then(handleNext)
      .catch(() => {
  ...
  async function validateDateFormat(columnName, pythonFormatString) {
    ...
    try {
      const response = await axios.post('/api/data_annotation/datasets/validate_date', payload);
